...
''')
    ...
    def test_summary_distinguishes_refusal_from_measured_threshold_failure(self):
        ...
        self.assertIn(b"raw low-address operands: REFUSED/UNAVAILABLE(exit=65)", refused.stdout)
        self.assertIn(b"raw absolute-ROM operands: REFUSED/UNAVAILABLE(exit=65)", refused.stdout)
        ...
        self.assertIn(b"raw low-address operands: 1\n", measured.stdout)
        self.assertIn(b"raw absolute-ROM operands: 1\n", measured.stdout)
        self.assertNotIn(b"REFUSED/UNAVAILABLE", measured.stdout)
        self.assertEqual(len(self.calls.read_text().splitlines()), 1)

    def test_standalone_maturity_once_with_ordinary_warnings(self):
        project = self.make_ci_fixture()
        env = self.counted_environment()
        refreshed = subprocess.run(["bash", "scripts/refresh_inventory.sh", "synthetic"], env=env, capture_output=True)
        self.assertEqual(refreshed.returncode, 0, refreshed.stderr)
        self.calls.write_text("")
        source = project / "asm/input.asm"
        source.write_text("Unused .EQU 42\n" + source.read_text())
        command = ["bash", "scripts/project_maturity_check.sh", "synthetic"]
        run = subprocess.run(command, env=env, capture_output=True)
        self.assertEqual(run.returncode, 0, run.stdout.decode() + run.stderr.decode())
        calls = [json.loads(line) for line in self.calls.read_text().splitlines()]
        self.assertEqual(len(calls), 1)
        self.assertNotIn("--Werror=unused-equ", calls[0])
        self.assertTrue(any(arg.startswith("--instruction-records-output=") for arg in calls[0]))
        self.calls.write_text("")
        for supplied in ({"NESREV_ANALYSIS_BUNDLE": ""}, {"NESREV_ANALYSIS_BUNDLE": "missing.json"},
                         {"NESREV_XREF_FILE": "missing.json"}):
            run = subprocess.run(command, env=dict(env, **supplied), capture_output=True)
            self.assertEqual(run.returncode, 65, run.stderr)
    ...
    def test_raw_measurement_failure_cannot_publish_or_certify_zeros(self):
        ...
        env = self.counted_environment()
        inventory = project / "docs/reverse_engineering/inventory"
        refresh = ["bash", "scripts/refresh_inventory.sh", "synthetic"]
        run = subprocess.run(refresh, env=env, capture_output=True)
        self.assertEqual(run.returncode, 0, run.stderr)
        before = {p.name: p.read_bytes() for p in inventory.iterdir() if p.is_file()}
        baseline = inventory / "pass/baseline_status.json"
        baseline.write_text('{"original": true}\n')
        original_baseline = baseline.read_bytes()
        policy = inventory / "kpis.conf"
        ...
        for output, exit_code in (("[raw-kpi] strict_active_raw_lowaddr=0\n[raw-kpi] strict_active_raw_absrom=0\n", 65),
            ...
            with self.subTest(output=output, exit_code=exit_code):
                leaf.write_text(f"print({output!r}, end='')\nraise SystemExit({exit_code})\n")
                run = subprocess.run(refresh, env=env, capture_output=True)
                self.assertEqual(run.returncode, 65, run.stdout.decode() + run.stderr.decode())
                self.assertEqual({p.name: p.read_bytes() for p in inventory.iterdir() if p.is_file()}, before)
                run = subprocess.run(["bash", "scripts/project_maturity_check.sh", "synthetic"],
                                     env=env, capture_output=True)
                self.assertEqual(run.returncode, 65, run.stderr)
                self.assertNotIn(b"maturity hard gates passed", run.stdout)
                run = subprocess.run(["bash", "scripts/project_maturity_summary.sh", "synthetic"],
                                     env=env, capture_output=True)
                self.assertEqual(run.returncode, 0, run.stderr)
                self.assertIn(b"raw low-address operands: REFUSED/UNAVAILABLE(exit=65)", run.stdout)
                self.assertIn(b"raw absolute-ROM operands: REFUSED/UNAVAILABLE(exit=65)", run.stdout)
                run = subprocess.run(["bash", "scripts/project_pass_prep.sh", "synthetic"],
                                     env=env, capture_output=True)
                self.assertEqual(run.returncode, 65, run.stdout.decode() + run.stderr.decode())
                self.assertEqual(baseline.read_bytes(), original_baseline)
                pending_policy = b"# Intake calibration pending.\n" + original_policy
                policy.write_bytes(pending_policy)
                run = subprocess.run(["python3", "scripts/kpi_ratchet_calibrate.py", str(project / "asm/input.asm"),
                                      str(policy)], env=env, capture_output=True)
                self.assertNotEqual(run.returncode, 0, run.stdout.decode() + run.stderr.decode())
                self.assertEqual(policy.read_bytes(), pending_policy)
