...
def render_implementation_note(root: Path, state: dict[str, Any], pass_id: str) -> str:
    commit_log = run_git(
        [
            "log",
            "--oneline",
...
def command_start_pass(args: argparse.Namespace) -> int:
    ...
    note_path.write_text(render_implementation_note(root, state, args.pass_id), encoding="utf-8")
