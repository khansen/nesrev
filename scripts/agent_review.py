...
from reference_review import planning_scope, validate_gold_assessment
...
def project_process_friction_path(root: Path, state: dict[str, Any]) -> Path:
    project = state.get("project")
    if not project:
        raise UserError("learning archive requires a project in state")
    return root / "projects" / project / "PROCESS_FRICTION.md"
...
def render_prompt(root: Path, state: dict[str, Any], role: str) -> str:
    ...
    if role == "reviewer":
        ...
        body = [
            ...
            "Include `## Reference Coverage`: assess the packet's reference scope against",
        ...
        if status == "APPROVED":
            body = [
                ...
                "After archiving, commit the review archive and any process-friction",
                "learning candidates before starting the next handoff.",
                "",
            ]
    ...
    if state.get("gold"):
        if role == "reviewer":
            body.extend([
                ...
                "Also include `## Reference Coverage` using the Sources, Inventory, Mappings,",
                "and Gaps fields in QUALITY_REVIEW.md#reference-coverage. Each field needs",
                "committed Markdown evidence links, including a no-gaps conclusion. Unresolved gameplay",
                "identities block gold. Record `Reference coverage: COMPLETE`, or",
                "`Reference coverage: EXPLICIT MANUAL WAIVER` with a linked Waiver field.",
...
def initial_state(
    ...
    *,
    ...
    reviewer: str,
    implementer: str,
    allow_process_range: bool,
    allow_unresolved_lxxxx: bool,
) -> dict[str, Any]:
    ...
    return {
        ...
        "last_response": None,
        "prompts": {},
        "allow_unresolved_lxxxx": allow_unresolved_lxxxx,
    }
...
def command_init(args: argparse.Namespace) -> int:
    ...
    state = initial_state(
        ...
        allow_process_range=args.allow_process_range,
        allow_unresolved_lxxxx=bool(args.allow_unresolved_lxxxx),
    )
    write_state(root, state)
    print(
        f"initialized {args.run_id} for "
        f"{state['review_base'][:10]}..{state['review_head'][:10]}"
    )
    return 0


def render_implementation_note(
    root: Path, state: dict[str, Any], pass_id: str, learning: str
) -> str:
    commit_log = run_git(
        [
            "log",
            "--oneline",
            "--reverse",
            f"{state['review_base']}..{state['review_head']}",
        ],
        cwd=root,
    ).strip()
    lines = [
        f"Implemented {state['project']} pass {pass_id}.",
        "",
        f"Reviewed range: {state['review_base']}..{state['review_head']}",
        "",
        "Commits:",
    ]
    if commit_log:
        lines.extend(f"- {line}" for line in commit_log.splitlines())
    else:
        lines.append("- (no commits in range)")
    lines.extend(
        [
            "",
            "Review packet is authoritative. This note was generated by",
            "`agent_review.py start-pass` to avoid manual handoff setup drift.",
            "",
            "## Reference Scope",
            "",
            "Unversioned planning context; verify against the committed range:",
            "",
            planning_scope(root / "projects" / state["project"] / "docs/reverse_engineering",
                           state["project"], pass_id),
            "",
            "## Learning Candidates",
            "",
            learning.strip() or "_None._",
            "",
        ]
    )
    return "\n".join(lines)
...
def command_start_pass(args: argparse.Namespace) -> int:
    root = repo_root()
    ...
    note_path.write_text(
        render_implementation_note(root, state, args.pass_id, args.learning),
        encoding="utf-8",
    )
    print(f"implementation note: {rel(root, note_path)}")
    write_state(root, state)

    ready_args = argparse.Namespace(
        note=rel(root, note_path),
        packet=None,
        generate_packet=True,
    )
    command_ready(ready_args)
    return command_status(argparse.Namespace(json=False))
...
def command_approve(args: argparse.Namespace) -> int:
    ...
    if state.get("gold"):
        ...
        try:
            validate_gold_assessment(text, root, state["review_head"], state["project"])
        except packet_evidence.PacketError as exc:
            raise UserError(str(exc)) from exc
        ensure_clean_tracked(root)
        ensure_head(root, state["review_head"])
...
def learning_section_body(text: str) -> str | None:
    ...
    while i < len(lines):
        ...
        if not learning_body_is_empty(body):
            bodies.append(body)
        i = body_end
    if not bodies:
        return None
...
def learning_artifacts(root: Path, state: dict[str, Any]) -> list[tuple[str, str]]:
    artifacts: list[tuple[str, str]] = []
    note = state.get("implementation_note")
    if note:
        note_path = resolve_path(root, note)
        if note_path.exists():
            artifacts.append((rel(root, note_path), note_path.read_text().rstrip()))
    artifacts.extend(read_run_artifacts(root, state, "review-*.md"))
    artifacts.extend(read_run_artifacts(root, state, "response-*.md"))
    return artifacts


def collected_learning_candidates(root: Path, state: dict[str, Any]) -> list[tuple[str, str]]:
    candidates: list[tuple[str, str]] = []
    for source, text in learning_artifacts(root, state):
        body = learning_section_body(text)
        if body:
...
def render_learning_block(
    ...
    lines = [
        start,
        f"### Pass {pass_id} - {state['run_id']}",
        "",
        f"Archive: `{archive_path}`",
        f"Review-time head: `{state['review_head']}`",
        "",
        "Raw learning candidates captured from agent-review artifacts. Triage",
        "through process review before promoting them to playbooks or scripts.",
        "",
    ]
    for source, body in candidates:
        lines.extend([f"#### {Path(source).name}", "", f"Source: `{source}`", "", body, ""])
    lines.append(end)
    return "\n".join(lines).rstrip() + "\n"


def learning_block_markers(state: dict[str, Any], pass_id: str) -> tuple[str, str]:
    key = f"{state['run_id']}:pass-{pass_id}"
    return (
        f"<!-- agent-review-learning:{key}:start -->",
        f"<!-- agent-review-learning:{key}:end -->",
    )
...
def command_archive(args: argparse.Namespace) -> int:
    ...
    archive_path = rel(root, out_path)
    out_path.write_text(render_archive(state, args.pass_id, archive_path))
    print(f"archived review artifacts to {rel(root, out_path)}")
    friction = update_process_friction(root, state, args.pass_id, archive_path)
    if friction:
        print(f"recorded learning candidates in {friction}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    start.add_argument("--reviewer", default="reviewer")
    ...
    start.add_argument(
        "--learning",
        default="_None._",
        help="Learning Candidates text for the generated implementation note",
    )
    start.set_defaults(func=command_start_pass)
