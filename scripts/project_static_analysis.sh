...
# Regenerate a project's STATIC_ANALYSIS.md from the structured
# analysis in scripts/static_analysis.py (dead instructions +
...
DOC_OUT="${DOC_ROOT}/STATIC_ANALYSIS.md"
# Title-case the slug: my_project_slug -> "My Project Slug"
...
python3 "${SCRIPT_DIR}/static_analysis.py" "${ASM_FILE}" \
  --doc-out "${DOC_OUT}" --title "${TITLE}" --commit "${COMMIT}" --date "${DATE}"

echo "wrote ${DOC_OUT}"
python3 "${SCRIPT_DIR}/static_analysis.py" "${ASM_FILE}" --print
