...
split_unknown_pointer_count="$(awk -F, 'NR > 1 && $5 == "unknown_pointer" { count++ } END { print count + 0 }' "${inv_dir}/split_pointer_targets.csv")"
if [ "${split_unknown_pointer_count}" != "0" ]; then
  cat >> "${inv_dir}/unknowns.md" <<DOC
- Review split_pointer_targets.csv entries with target_type=unknown_pointer and classify each as code/data/mixed.
DOC
fi
