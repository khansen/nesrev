...
def bootstrap(role: str, project: str, root: Path, task_path: Path) -> str:
    ownership = (
        ...
        "Every semantic review checks the reference scope, source inventory, crosswalk mappings, and "
        "newly answerable identities; justified WIP gaps need concrete evidence conditions. "
...
def kickoff(root: Path, project: str, task: str) -> str:
    ...
    return (
        ...
        "the working notes, source-term inventory, crosswalk, and an explicitly selected corridor.\n"
        "Record REFERENCE_SCOPE with the corridor plan: relevant source concepts, existing mappings, "
        "and identity questions this pass can resolve (or why none apply). Name proven identities "
        "and update the crosswalk in the same pass; preserve shared machinery names. "
        "Revisit identity debt when later passes supply evidence; do not wait for gold review.\n"
