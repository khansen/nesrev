...
9. **Major data regions have dedicated format coverage.** Inline `Format:`
   ...
   entries already expose both facts. The standard four-byte hardware OAM
   record alone does not require a project format doc; use the canonical
   `OAM_FIELD_*` layout and document only project-specific template or
   metasprite semantics locally.
